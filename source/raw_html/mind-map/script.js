function hexToRgb(hex) {
  ...
  // Get ?post=post1 from the URL
const params = new URLSearchParams(window.location.search);
const post = params.get('post') || 'post1';
...
fetch(`content/${post}.md`)
  ...
  .then(markdown => {
    ...
    document.getElementById("post-title").textContent = post.replace(/-/g, " ");
  })
  .catch(err => {
    document.getElementById("post-container").innerHTML = "<p>That entry could not be summoned.</p>";
  });
